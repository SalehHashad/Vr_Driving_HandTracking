...
public class RCC_CarControllerV3 : RCC_Core {
	...
	public bool SteeringBtnUp = false;
	public bool SteeringBtnDown = false;
	#endregion
	...
	public Gear[] gears;
	public int totalGears = 5;			//	Total count of gears.
	public int currentGear = 0;		// Current Gear Of The Vehicle.
	public bool NGear = false;      // N Gear.
	public bool NGearWheel = false; //temp Ngear
	public bool Manual1stGear = true;
	...
	private bool clutchStart = false;
	...
	public bool automaticGear{get{if(!externalController) return RCCSettings.useAutomaticGear; else return true;}}
	//public bool automaticGear = true;
	// Enables / Disables automatic gear shifting.//will see
	...
	#endregion
	//added for driving mood change
	public bool ModeChange = false; // default mode
	public bool ResetGears = false;
	//
	...
	private AudioClip hornClip { get { return RCCSettings.hornClip; } }
	...
	//added
	[HideInInspector]public int gearInput = 0;
	...
	void Awake (){
		//Debug.Log("RCC : " + RCCSettings.useAutomaticGear);
	...
	void OnEnable(){
		//Debug.Log("ChangingGear ? " + changingGear);
		changingGear = false;	
    ...
    private void CreateAudios(){
		...
		hornSound = NewAudioSource(RCCSettings.audioMixer, gameObject, "Horn Sound AudioSource", 2, 25, 0, hornClip, true, true, false); //added
	...
	public void InitGears (){
		//values should be reversed! --added 
		gears = new Gear[totalGears];
		Debug.Log("total Gears: " + totalGears);
		...
		if (gears.Length == 7) //flip values!
			gearRatio = new float[] { .8f, .9f, 1.0f, 1.23f, 1.66f, 2.5f, 4.5f };
		...
		//Debug.Log("gears[currentGear]: " + gears[currentGear].maxRatio);
	}
	...
	void Update (){
		...
		//Reversing Bool.
		//if (!externalController){
	//	Debug.Log("here : " + "gearInput: " + gearInput + " direction: " + direction);
		//------FOR AUTOMATIC GEAR ONLY ------//
		if ( gearInput == 5 && transform.InverseTransformDirection(rigid.velocity).z < 1f && canGoReverseNow && automaticGear && !semiAutomaticGear && !changingGear && direction != -1)
		{
			//Debug.Log("in brake conidion");
			
				StartCoroutine(ChangeGear(5));
		}
		
		else if (gearInput != 5 && transform.InverseTransformDirection(rigid.velocity).z > -1f && direction == -1 && !changingGear && automaticGear && !semiAutomaticGear)
        {
			
			//Debug.Log("in forward conidion");
	...
	//hna
		if (canControl) {
			...
			if (!externalController) {
				...
				//Debug.Log("RCC_LogitechSteeringWheel.GetKeyTriggered(0, RCCSettings.LogiSteeringWheel_shiftGearUp) : " + RCC_LogitechSteeringWheel.GetKeyTriggered(0, RCCSettings.LogiSteeringWheel_shiftGearUp));
				//if rcc btn is prssed
				//set bool equal false
				//
				NGearWheel = inputs.Ngear;
				...
				if (!automaticGear || semiAutomaticGear) {
					...
					{//FOR AUTOMATIC CONTROL
					 ...
					 //throttleInput = (direction == 1 ? Mathf.Clamp01(inputs.throttleInput) : Mathf.Clamp01(inputs.brakeInput));
						...
						//throttleInput = Mathf.Clamp01(inputs.brakeInput);
						if ( gearInput != 5)
                        {
							throttleInput = (direction == 1 ? Mathf.Clamp01(inputs.throttleInput) : Mathf.Clamp01(inputs.brakeInput));
						}
				...
				else 
				{
                    ...
                    {//FOR AUTOMATIC CONTROL
						...
						//Debug.Log("my brake value " + direction +"  "+ brakeInput);
						// for test 
						// break should not equal throttle at all
						//brakeInput = Mathf.Clamp01(inputs.brakeInput);
						//Debug.Log("Automatic");
                        if (gearInput != 5)
                        {
	...
	private void GetExternalInputs(){
		...
		case RCC_Settings.ControllerType.LogitechSteeringWheel:
			...
			if (log) {
				...
				if(RCC_LogitechSteeringWheel.GetKeyTriggered(0, RCCSettings.LogiSteeringWheel_startEngineKB))
					...
					//Debug.Log(RCCSettings.LogiSteeringWheel_horn);
					if (RCC_LogitechSteeringWheel.GetKeyPressed(0, RCCSettings.LogiSteeringWheel_horn))
                    {
						hornSound.volume = 24f;
						//Debug.Log("beep beep");
					}
                    else
                    {
                    ...
                    if (IsBumpy)
                    {
						RCC_LogitechSteeringWheel.PlayBumpyRoadEffect();

					}
					else
                    {
						RCC_LogitechSteeringWheel.StopBumpyRoadEffect();

					}
					

				// triangle button opens waiting lights
					...
					if(indicatorsOn != IndicatorsOn.All){
					   ...
					   // traingle button closes waiting lights
						indicatorsOn = IndicatorsOn.Off;
					...
					//NGear = true; //added for test
					//
					
				if(!automaticGear){
						//btns behind the wheel
					...
					if(RCC_LogitechSteeringWheel.GetKeyTriggered(0, RCCSettings.LogiSteeringWheel_shiftGearDown))
						...
						if(currentGear != gearInput)
                        {
							...
							if (gearInput != -1)
                            {
                                if (!useAutomaticClutch)
                                {
									...
									if( clutchInput >= 0.85f)
                                    {
										//Debug.Log("Currentgear : " + currentGear + " and gearInput : " + gearInput + "gears[currentGear].maxRatio : " + gears[currentGear].maxRatio);
										Manual1stGear = true;
										//Debug.Log("changing gears 1");
										StartCoroutine(ChangeGear(gearInput));
									}
						...
						if (currentGear == 0 && gearInput == 0 && Manual1stGear)
						{
							StartCoroutine(ChangeGear(gearInput));
							//Debug.Log("Hello");
						//	Debug.Log("HIII  Currentgear : " + currentGear + " and gearInput : " + gearInput + "gears[currentGear].maxRatio : " + gears[currentGear].maxRatio);
							Manual1stGear = false;
						}
       ...
       //             }
		...
		//Debug.Log("permenant gas is : " + permanentGas); false
		if (permanentGas)
			throttleInput = 1f;
	...
	void ControlGears()
    {
		if(automaticGear !=  ModeChange)
        {
			//Debug.Log("clutch : " + clutchStart);
			ModeChange = automaticGear;
			ResetGears = true;
			clutchStart = true;
        }
    }
	...
	void FixedUpdate (){
		...
		oldMaxEngineRPM = maxEngineRPM;
		ControlGears();
		//Debug.Log("gears: " + gears + " gears.Length: " + gears.Length + " ResetGears: " + ResetGears);
		if (gears == null || gears.Length == 0 || ResetGears) {

			//Debug.Log(" Reset Gears ");
			print ("Gear can not be 0! Recreating gears...");
			gears = new Gear[totalGears];
			Debug.Log("Auto ? " + automaticGear);
			if (automaticGear)
            {
				totalGears = 5;
			}
		...
		if (canControl) {
            ...
            if (useAutomaticClutch) // changed from if(useAutomaticClutch)
                Clutch();
  ...
  //      {
        ...
        //added
        //Add Road effects on wheel

        //1- Bumpy Road\
        if (FrontLeftWheelCollider.wheelHit.collider != null && FrontRightWheelCollider.wheelHit.collider != null)
        {


            if (FrontLeftWheelCollider.wheelHit.collider.tag == "BumpyRoad" && FrontRightWheelCollider.wheelHit.collider.tag == "BumpyRoad")
            {
                IsBumpy = true;
                //Debug.Log("BUMPYYY ROOOAD");
            }
            else if (FrontLeftWheelCollider.wheelHit.collider.tag != null && FrontRightWheelCollider.wheelHit.collider.tag != null)
            {
            ...
            //if (FrontLeftWheelCollider.wheelHit.collider.tag != null && FrontRightWheelCollider.wheelHit.collider.tag != null)
            //{
            //}


           // Debug.Log(FrontLeftWheelCollider.wheelHit.collider);
            //Debug.Log("IS bumpy : " + IsBumpy);
	...
	private void Engine (){
		...
		float velocity = 0f;
		//hena2
		//Debug.Log("gears ratio : " + gears[currentGear].maxRatio);
	    ...
	    //Debug.Log("clutchInput * throttleInput" + clutchInput);

        if (clutchStart)
        {
			if (clutchInput > 0.3f && clutchInput < 1f && throttleInput > 0.1f)
            {
				clutchStart = false;
			//	Debug.Log("clutch " + clutchStart);
			}
        ...
        {
			//Debug.Log("Current gear in engine : " + currentGear);
			engineRPM = Mathf.SmoothDamp(engineRPM, (Mathf.Lerp(minEngineRPM, maxEngineRPM + 500f, (clutchInput * throttleInput)) + ((Mathf.Abs(wheelRPM / Mathf.Clamp(poweredWheels, 1, Mathf.Infinity)) * finalRatio * (gears[currentGear].maxRatio)) * (1f - clutchInput))) * fuelInput, ref velocity, engineInertia);
			engineRPM = Mathf.Clamp (engineRPM, 0f, maxEngineRPM + 500f);
		}
		...
		else
        {
            if (automaticGear)
            {
				if (gearInput == 5 || RCC_LogitechSteeringWheel.GetKeyTriggered(0, RCCSettings.LogiSteeringWheel_shiftGearUp))
					canGoReverseNow = true;
        ...
        //Add new condition using gearinput
		//Make reverse for automatic with gearinput 6
		//make reverse for manual with gearinput -1
  //      else if (gearInput == -1) {
	...
	private void Wheels(){
		...
		for (int i = 0; i < allWheelColliders.Length; i++) {
			...
			//Debug.Log("appliedBrake : " + appliedBrake);
	...
	private void Clutch(){
		...
		if (currentGear == 0) {
			...
			//Debug.Log("Clutch gear + changingGear :" + changingGear);
			if (changingGear)
            {

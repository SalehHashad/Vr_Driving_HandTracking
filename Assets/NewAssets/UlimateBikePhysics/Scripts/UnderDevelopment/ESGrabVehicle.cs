...
public class ESGrabVehicle : MonoBehaviour
{
    ...
    public Transform RightFootIkTarget, LeftHint, RightHint;
    ...
    private bool GrabbedLeft, GrabbedRight, movetopos, callragdollz, returnreverseanim, AIGrab,
       Stop, SingleClick, Umount, pickfrmright, pickfrmleft, InRange;
    #endregion
    private void Awake()
    {
        StopCameraRender();
    ...
    void OnDrawGizmos()
    {
        ...
        if (RightHint != null)
        {
            Gizmos.DrawWireSphere(RightHint.position, .075f);
        }
        if (LeftHint != null)
        {
            Gizmos.DrawWireSphere(LeftHint.position, .075f);
        }
        if (SitPoint != null)
        {
    ...
    private void ParentToVehicleOnStart()
    {
        if (SpawnAIWithVehicle)
        {
            if (returnUpdate)
            {
                ...
                //AiTransform.GetComponent<ESIkRider>()._Exit_Vehicle = this.GetComponent<ESGrabVehicle>();
                //SetRiderIK();
                ...
                //AiTransform.GetComponent<ESIkRider>().MotorRigid = motorCycleController;
                returnUpdate = false;
            }
        }
    }
    ...
    private void FixedUpdate()
    {
        ...
        {
            if (motorCycleController.CurrentSpeed < 0.1f)
            {
            ...
            if (Player.GetComponent<ESRagDollHelper>().Ragdollz)
            {
                if (Player.GetComponent<ESRagDollHelper>().BoneState == "Calm")
                {
                    if (callragdollz)
                    {
                        ...
                        Player.GetComponent<ESIkRider>().ignore = false;
                        InRange = false;
                        callragdollz = false;
                    }
                }
    ...
    private void FixLocalEuler()
    {
        ...
        if (SitPoint != null)
        {
            /*
            Transform vehicletransform = _vehicleType == VehicleType.Bike ? motorCycleController.transform : null;
            Vector3 siteuler = SitPoint.localEulerAngles;
            if (StaticLocalEuler == StaticLocalEulerAngles.BothAxis)
            {
                siteuler.x = -1 * vehicletransform.transform.eulerAngles.x;
                siteuler.z = -1 * vehicletransform.transform.eulerAngles.z;
            }
            else if (StaticLocalEuler == StaticLocalEulerAngles.X_AxisOnly)
            {
                siteuler.x = -1 * vehicletransform.transform.eulerAngles.x;
            ...
            SitPoint.localEulerAngles = siteuler;
            */
        }
        #endregion
    }
    ...
    private void SetRiderIK()
    {
        ...
        ikRider.LeftHint = LeftHint;
        ikRider.RightHint = RightHint;
    ...
    private void OnTriggerEnter(Collider Other)
    {
        if (Other.CompareTag("Player"))
        {
            ...
            {
                // print("Player");
                if (!Other.GetComponent<ESIkRider>().ignore)
                {
                    InRange = true;

                    Player = PlayerTransform;
                    Player.GetComponent<ESIkRider>().ignore = true;
                }
            }
        }
    }
    ...
    private void PerformPickOperationOnBike()
    {
        ...
        if (attemptpick)
        {
            motorCycleController.callreset = false;
            if (motorCycleController.LooseControl)
            {
                motorCycleController.MotorRb.isKinematic = true;
                Animator animator = Player.GetComponent<Animator>();
                if (motorCycleController.eulerz > 0)
                    animator.SetBool("MirrorBlend", true);
                else
                    animator.SetBool("MirrorBlend", false);
                //
                animator.SetFloat("PickBikeEulerInverse", Mathf.Abs(motorCycleController.eulerz));
    ...
    private void UnmoutVehicle()
    {
        ...
        if (AIGrab) return;
        if (Input.GetKeyDown(GrabKey) || motorCycleController.callreset)
        {
            // PlayerTransform.GetComponent<Animator>().ResetTrigger("Cluch");
            Vector3 bikeeuler = transform.eulerAngles;
            bikeeuler.z = 0;
            transform.eulerAngles = bikeeuler;
            if (!AIGrab)
            ...
            Animator animator = Player.GetComponent<Animator>();
            animator.applyRootMotion = true;
            string Exit = motorCycleController.MotorRb.velocity.sqrMagnitude > 0.0025f ? AgressiveExit : ExitVehile;
            if (Exit == AgressiveExit)
            {
                callragdollz = true;
                Player.GetComponent<ESRagDollHelper>().AddbusrtEffect = true;

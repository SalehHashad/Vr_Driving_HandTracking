...
public class ESIkRider : MonoBehaviour
{
    ...
    [HideInInspector] public Transform LeftIkTarget, RightIKTarget, RightFootIKTarget, LeftFootIKTarget, LeftHint, RightHint;
    [HideInInspector] public UL_MotorCycleController MotorRigid;
    [HideInInspector] public bool ignore;
    //[HideInInspector]
    public ESGrabVehicle _Exit_Vehicle;
    [Header("DEBUG")]
    public Animator animator;
    [SerializeField] private Transform RightFoot, LeftFoot;
    [Header("Base On MotorCycle Preset")]
    [SerializeField] private float ReverseBlend = 0.5f, Ymod = 0.091f;
    ...
    [HideInInspector] public bool OreintLeg, OreinlegsInreverse, firstleg, animatedstunts;
    ...
    void FixedUpdate()
    {
        ...
        if (LeftFootIKTarget != null && MotorRigid != null)
        {
            ...
            if (Physics.Raycast(Lpos, -Vector3.up, out Lhit, legdist))
            {
        ...
        if (RightFootIKTarget != null && MotorRigid != null)
        {
            ...
            Vector3 offsetvec = !MotorRigid.IsDrifting ? LegOffset : DriftLegOffset;
            if (Physics.Raycast(Rpos, -Vector3.up, out Rhit, legdist))
            {
    ...
    //
    void UpdateLegIkBasedOnReverse()
    {
        if (_Exit_Vehicle.BikeBackardZ < 0)
        {
            animator.SetIKPositionWeight(AvatarIKGoal.RightFoot, ReverseBlend);

            //
            animator.SetIKPosition(AvatarIKGoal.RightFoot, Rpos + new Vector3(0, Ymod, 0));
            //
            animator.SetIKPositionWeight(AvatarIKGoal.LeftFoot, ReverseBlend);

            //
            animator.SetIKPosition(AvatarIKGoal.LeftFoot, Lpos + new Vector3(0, Ymod, 0));
        }
    }
    //

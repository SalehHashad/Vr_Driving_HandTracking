...
public class UL_BikeSkid : MonoBehaviour
{
    [System.Serializable]
    public class GenerateSkid
    {
        ...
        [HideInInspector] public bool ApplySound_Smoke = false;
        [HideInInspector]
        ...
        public void Addskidmarks(Ul_Suspension collider, Transform Curtransform)
        {
            RaycastHit hit;
            //collider.GetGroundHit(out hit);
            hit = collider.wheelhit;
    ...
    void Start()
    {
        ...
        wheelcollider = GetComponent<Ul_Suspension>();
    ...
    void LateUpdate()
    {
        ...
        {
            burnoutparticle.transform.position = transform.position - transform.up * wheelcollider.WheelRaduis;
        }
        ...
        if (Callskid)
        {
            ...
            if (wheelcollider.m_isgrounded)
            {
                if (generateskid.ApplySound_Smoke)
                {
                    DoSkidSound(true);
                    if (burnoutparticle != null)
                    {
